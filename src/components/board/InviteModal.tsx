...
export function InviteModal({ workspaceId, onClose }: Props) {
  ...
  async function handleInvite(e: React.FormEvent) {
    ...
    try {
      ...
      // Workspace tipini otomatik 'shared' yap (eğer personal ise)
      ...
      toast.success(data?.message || 'Kullanıcı davet edildi')
      setTimeout(() => onClose(), 1500)
    } catch (err: any) {
      toast.error(err.message || 'Davet edilemedi.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden flex flex-col">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="font-semibold text-gray-800">Çalışma Alanına Davet Et</h2>
        ...
        <form onSubmit={handleInvite} className="p-5 flex flex-col gap-4">
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1.5 ml-1">Kullanıcı E-posta Adresi</label>
            <input
              type="email"
              required
              value={email}
            ...
            />
            <p className="text-[11px] text-gray-400 mt-1.5 ml-1">Kullanıcı kayıtlıysa dogrudan eklenir, değilse davet beklemeye alınır.</p>
          </div>
          ...
          <div className="flex justify-end gap-2 mt-2">
            ...
            >
              {loading ? 'Gönderiliyor...' : <><Send size={14}/> Davet Gönder</>}
            </button>
          </div>
        </form>
      </div>
